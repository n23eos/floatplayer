...
## Скриншоты (готовы, 1280×800, `store/assets/`)

| Файл | Кадр |
|---|---|
| `screenshot-1-always-on-top.png` | окно поверх рабочего приложения |
| `screenshot-2-controls.png` | панель управления с выносками |
| `screenshot-3-sponsorblock.png` | зелёный сегмент + кнопка пропуска |
| `screenshot-4-recommendations.png` | открытая колонка рекомендаций |
| `screenshot-5-shorts.png` | вертикальное окно шортса |
| `screenshot-6-options.png` | страница настроек (реальный рендер) |

Кадры 1–5 собраны на настоящих стилях расширения (`pip/pip.css`) и его
классах — интерфейс на них соответствует продукту; вместо чужого видео
нейтральный градиент (чтобы не использовать чужой контент). Кадр 6 — прямой
рендер реальной страницы настроек.

Промо-тайл: `store/assets/promo-tile-440x280.png`.

Опционально: заменить кадры 1–5 живыми captures с работающего расширения
(macOS: Cmd+Shift+4 по окну, затем обрезать/подложить под 1280×800).
